(() => {
  ...
  drawMarker=function(cluster,relative){
    ...
    if(lightweight){
      ...
      ctx.strokeStyle=inRoute?'#edc574':'rgba(255,244,226,.72)';
      ctx.stroke();
    ...
    ctx.shadowColor=selected?'rgba(224,60,72,.62)':'rgba(0,0,0,.5)';
    ctx.shadowBlur=selected?18:7;
    ctx.shadowOffsetY=selected?1:3;
    tracePin(ctx,cluster.x,cluster.y,radius+2.5,tipHeight+2.2);
    ctx.fillStyle=selected?'rgba(255,245,228,.96)':'rgba(18,16,15,.9)';
    ctx.fill();

    ctx.shadowColor='transparent';
    tracePin(ctx,cluster.x,cluster.y,radius,tipHeight);
    const gradient=ctx.createRadialGradient(cluster.x-radius*.35,centerY-radius*.42,1,cluster.x,centerY,radius*1.08);
    gradient.addColorStop(0,lighten(base,26));
    gradient.addColorStop(1,base);
    ctx.fillStyle=gradient;
    ctx.fill();
    ctx.lineWidth=selected?2.3:1.15;
    ctx.strokeStyle=inRoute?'#edc574':selected?'#fff7e8':'rgba(255,239,218,.74)';
    ctx.stroke();
    ...
    if(favorite){
      ctx.beginPath();
      ctx.arc(cluster.x+radius*.72,centerY-radius*.74,3.8,0,Math.PI*2);
    ...
    if(selected){
      tracePin(ctx,cluster.x,cluster.y,radius+7,tipHeight+4.5);
      ctx.strokeStyle='rgba(236,74,86,.65)';
      ctx.lineWidth=2;
      ctx.stroke();
    }
    ctx.restore();
  ...
  window.Atlas080={animateButtonZoom,cancelButtonZoom,tracePin};
  scheduleDraw();
})();