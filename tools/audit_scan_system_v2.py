...
def main() -> int:
    ...
    health = read_text("scan-monitor-health.js")
    ...
    check("monitor_poll", "FAST_POLL_MS=10000" in monitor and "CHECK_MS=10000" in health, "10-second page checks")
    check("monitor_thresholds", "RUNTIME_FRESH_MS=150000" in monitor and "RUNNING_STALE_MS=180000" in monitor and "EXPECTED_HEARTBEAT_SECONDS=60" in health, "60/150/180-second thresholds")
    check("monitor_versions", "VERSION='0.3.1'" in monitor and "VERSION='0.3.2'" in health and "scan-monitor-health.js?v=0.3.2" in monitor_html, "monitor core and health versions")
    check("monitor_schema", "validRuntime" in monitor and "invalid runtime schema" in health and "invalid status schema" in health, "runtime and status schema guards")
    check("monitor_completion_priority", "if(status?.complete)" in health and "matchedRuntime(status,runtime)" in health, "completion overrides stale runtime")
    check("monitor_cache", "monitor-v5" in worker and "scan-monitor-health.js" in worker, "cache generation v5")
