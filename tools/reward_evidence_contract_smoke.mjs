...
const rewardSearchValid=runtimeSource.includes('function rewardAwareSearch')&&runtimeSource.includes('function rewardSearchDocument')&&runtimeSource.includes('window.runSearch = rewardAwareSearch')&&runtimeSource.includes('atlas-search-reward')&&locationSearchPatch.includes("const OWNER = 'reward-aware-bilingual-search-v2'")&&locationSearchPatch.includes('titleAliases: [location.title_zh, location.title, location.title_en]')&&locationSearchPatch.includes('const aliases = [reward.nameOriginal, reward.nameZhCN]')&&locationSearchPatch.includes('item.aliases.includes(query.normalized)')&&locationSearchPatch.includes("kind = 'reward-exact'")&&locationSearchPatch.includes("kind = 'location-exact'")&&locationSearchPatch.includes('right.tier - left.tier')&&locationSearchPatch.includes('const matchedRewards = new Map()')&&locationSearchPatch.includes('命中奖励：')&&locationSearchPatch.includes('escapeHTML(item.rewardNames')&&locationSearchPatch.includes('function focusAfterViewportSettles')&&locationSearchPatch.includes('const visual = window.visualViewport')&&locationSearchPatch.includes('stableFrames >= 2')&&locationSearchPatch.includes('String(item.id) === String(button.dataset.id)')&&locationSearchPatch.includes('AtlasSearchOwner')&&locationSearchPatch.includes("window.AtlasRewards.refresh().then(install, install)")&&html.includes('搜索地点、区域、奖励或类别')&&rewardCss.includes('.atlas-search-reward')&&rewardCss.includes('.atlas-search-match');
...
const contracts={
  ...
  rewardOwner:manifest.runtimeOwners?.rewardEvidenceIndex==='data/rewards/reward-evidence-index.json'&&manifest.runtimeOwners?.rewardSummaryRuntime==='atlas-rewards-0949.js'&&manifest.runtimeOwners?.rewardRuntimeRecords==='data/rewards/reward-records-runtime.json'&&manifest.runtimeOwners?.locationSearch==='location-search-patch.js',
  rewardMatrix:manifest.invariants?.requiredRewardEvidenceChecks===500&&manifest.invariants?.rewardUnresolvedNeverFabricated===true&&manifest.invariants?.rewardRuntimeRecordCount===runtimeIndex.recordCount&&manifest.invariants?.rewardSingleSourceMustRemainInference===true&&manifest.invariants?.rewardPartialSourceCoverageMustRemainInference===true&&manifest.invariants?.rewardSourceLineageMustBeQualified===true&&manifest.invariants?.rewardCandidateRecordsMachineChecked===true&&manifest.invariants?.singleLocationSearchController===true&&manifest.invariants?.rewardSearchExactMatchesFirst===true&&manifest.invariants?.rewardSearchMatchedRewardOnly===true&&manifest.invariants?.rewardSearchWaitsForViewportSettle===true&&runtimeRecordsValid&&reviewQueueValid&&runtimeCoverageValid&&rewardSearchValid&&runtimeIndex.noteZhCN.includes('数据链独立性')&&runtimeSource.includes('旧描述不会被当作已确认事实')&&runtimeSource.includes('loadRuntimeRecords')&&runtimeSource.includes('Promise.allSettled')&&runtimeSource.includes('rewardSupportCount')&&runtimeSource.includes('个来源')&&serviceWorker.includes(String.raw`records\/[^?]+\.json`)&&serviceWorker.includes('location-search-patch|page-zoom-guard'),
