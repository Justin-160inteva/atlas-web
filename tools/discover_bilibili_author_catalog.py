#!/usr/bin/env python3
"""Discover an authorized Bilibili author's AC Shadows catalog.
...
def get(url):
    response=requests.get(url,headers=HEADERS,impersonate="chrome",timeout=35); response.raise_for_status(); payload=response.json()
    if payload.get("code")!=0: raise RuntimeError(f"code={payload.get('code')} {payload.get('message')}")
    return payload

def duration_seconds(value):
    if isinstance(value,str) and ":" in value:
        parts=[int(p) for p in value.split(":") if p.isdigit()]; return sum(v*(60**i) for i,v in enumerate(reversed(parts)))
    try: return int(value or 0)
    except (TypeError,ValueError): return 0

def video(raw,author):
    bvid=clean(raw.get("bvid") or raw.get("id"))
    if not bvid:
        match=re.search(r"(BV[0-9A-Za-z]+)",clean(raw.get("arcurl") or raw.get("url"))); bvid=match.group(1) if match else ""
    ...
    return {"bvid":bvid,"title":clean(raw.get("title")),"author":author,"durationSeconds":duration_seconds(raw.get("duration") or raw.get("length")),"publishedAtUnix":int(raw.get("created") or raw.get("pubdate") or raw.get("timestamp") or 0),"url":f"https://www.bilibili.com/video/{bvid}/"}
...
def seed_info(bvid):
    data=get(f"https://api.bilibili.com/x/web-interface/view?bvid={quote(bvid)}").get("data") or {}; owner=data.get("owner") or {}
...
def discover_space_api(mid,author,diagnostics):
    ...
    for template in ("https://api.bilibili.com/x/space/arc/search?mid={mid}&ps=50&pn={page}&order=pubdate","https://api.bilibili.com/x/space/wbi/arc/search?mid={mid}&ps=50&pn={page}&order=pubdate"):
        imported=0
        try:
            for page in range(1,41):
                data=get(template.format(mid=mid,page=page)).get("data") or {}; rows=((data.get("list") or {}).get("vlist") or [])
                if not rows: break
                for raw in rows:
                    item=video(raw,author)
                    if item: found[item["bvid"]]=item; imported+=1
                count=int((data.get("page") or {}).get("count") or 0)
                if count and page*50>=count: complete=True; break
            if imported: diagnostics.append(f"space api imported {imported}"); complete=True; break
        except Exception as exc: diagnostics.append(f"space api failed: {exc!r}"[-1200:])
    return found,complete

def discover_ytdlp(mid,author,diagnostics):
    command=["yt-dlp","--flat-playlist","--dump-single-json","--playlist-end","500",f"https://space.bilibili.com/{mid}/video"]
    try:
        result=subprocess.run(command,cwd=ROOT,capture_output=True,text=True,timeout=240)
        if result.returncode!=0: raise RuntimeError((result.stderr or result.stdout)[-1500:])
        payload=json.loads(result.stdout); found={}
        for raw in payload.get("entries") or []:
            item=video(raw,author)
            if item: found[item["bvid"]]=item
        diagnostics.append(f"yt-dlp space imported {len(found)}")
        return found,bool(found)
    except Exception as exc:
        diagnostics.append(f"yt-dlp space failed: {exc!r}"[-1500:]); return {},False
    ...
    for query in (f"{author} 刺客信条影",f"{author} 刺客信条：影",f"{author} AC Shadows"):
        try:
            for page in range(1,11):
                payload=get(f"https://api.bilibili.com/x/web-interface/search/type?search_type=video&page={page}&page_size=50&keyword={quote(query)}"); rows=list(walk(payload.get("data")))
                if not rows: break
                accepted=0
                for raw in rows:
                    item=video(raw,author)
...
def classify(title,duration):
    text=norm(title); core=["全任务","全收集","全流程","流程","探索","区域","支线","主线","开放世界"]; places=["收集","地点","位置","城堡","寺庙","神社","古坟","九字真言","技能点","秘道","瞭望点"]; low=["boss","配装","武器","流派","伤害","秒杀","结局","片尾","纯剧情"]
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("manifest"); args=parser.parse_args(); manifest=load((ROOT/args.manifest).resolve()); diagnostics=[]
