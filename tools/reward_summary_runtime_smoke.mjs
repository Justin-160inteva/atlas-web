...
check(records.schemaVersion === 1, 'record index schema is supported');
check(records.recordCount === Object.keys(records.records).length, 'record count matches records object');
check(Array.isArray(records.reviewQueue), 'review queue is an array');
...
check(records.noteZhCN.includes('没有正式记录的点位保持 unresolved'), 'record index preserves unresolved state');
...
console.log(`reward summary runtime smoke passed: ${checks} checks`);
