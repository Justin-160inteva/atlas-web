#!/usr/bin/env python3
"""Atlas Conflict Reasoner v3.

Semantic release gate for Atlas. It distinguishes component metadata from code
that can change runtime state, and writes both JSON and Markdown reports.
"""
...
DIRECT_ALPHA_WRITE_RE = re.compile(r'\.textContent\s*=\s*[`"\'][^`"\']*ALPHA\s+(0\.\d+\.\d+\.\d+)', re.I)
...
class Reasoner:
    ...
    def check_release(self) -> None:
        ...
        # Component-local labels such as “资料库 0.9.1.3” are allowed. Only code
        # assigning an ALPHA string via textContent is treated as a release writer.
        for name, text in self.files.items():
            if name in {owner, 'index.html'} or not name.endswith('.js'):
                continue
            versions = sorted(set(DIRECT_ALPHA_WRITE_RE.findall(text)))
            if versions:
                self.add('critical', 'VERSION_OWNER_CONFLICT', f'{name} writes release label versions {versions}; only {owner} may own it', [name, owner], versions=versions)
    ...
    def check_ownership(self) -> None:
        ...
        for name in [name for name in sw_writers if name != sw_owner]:
    ...
    def check_navigation(self) -> None:
        ...
        if not match:
            ...
            nav = match.group(1)
            buttons = re.findall(r'<button\b[^>]*class=["\'][^"\']*\bnav-item\b[^"\']*["\'][^>]*>(.*?)</button>', nav, re.I | re.S)
            if len(buttons) != 5:
                self.add('critical', 'BOTTOM_NAV_BUTTON_COUNT', f'Expected 5 bottom navigation buttons, found {len(buttons)}', ['index.html'], count=len(buttons))
            for number, block in enumerate(buttons, start=1):
                host = re.search(r'<span\b[^>]*>(.*?)</span>', block, re.I | re.S)
                if not host:
                    self.add('critical', 'BOTTOM_NAV_ICON_HOST_MISSING', f'Button {number} has no icon host', ['index.html'], button=number)
                    continue
                text = re.sub(r'<[^>]+>', '', host.group(1)).strip()
                if text:
                    self.add('critical', 'LEGACY_NAV_TEXT_ICON', f'Button {number} contains legacy text icon {text!r}', ['index.html'], button=number)
