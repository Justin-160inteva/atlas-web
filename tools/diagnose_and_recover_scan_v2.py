...
def _enable_adaptive_transfer(manifest,*,parallel=True):
    manifest['analyzer']='tools/analyze_authorized_video_v12.py';opt=manifest.setdefault('downloadOptimization',{})
    opt.update({'noArtificialRateLimit':True,'adaptiveParallelRanges':bool(parallel),'maxRangeWorkers':max(2,min(6,int(opt.get('maxRangeWorkers',6)))),'parallelRangeThresholdBytes':int(opt.get('parallelRangeThresholdBytes',16777216)),'chunkSizeBytes':int(opt.get('chunkSizeBytes',2097152)),'rangeRetries':max(3,int(opt.get('rangeRetries',3))),'heartbeatSeconds':min(20,int(opt.get('heartbeatSeconds',20))),'speedWindowSeconds':min(30,int(opt.get('speedWindowSeconds',30))),'fallbackToResumableSingleStream':True,'preferBackupUrlOnRepeated412':True})
    return {'analyzer':manifest['analyzer'],'adaptiveParallelRanges':opt['adaptiveParallelRanges'],'maxRangeWorkers':opt['maxRangeWorkers'],'fallbackToResumableSingleStream':True}
def apply_action(action,active,manifest):
    ...
    if action in {'increase_backoff_and_retry_public_api','fast_backoff_public_api_and_adaptive_range'}:
        backoff=min(cap,max(floor,20*(2**max(0,attempts-1))));manifest['downloadBackoffSeconds']=backoff;manifest['preferPublicApi']=True;changed={'downloadBackoffSeconds':backoff,'preferPublicApi':True}
        if action=='fast_backoff_public_api_and_adaptive_range':changed.update(_enable_adaptive_transfer(manifest,parallel=True))
    ...
    elif action in {'use_v10_response_compatibility_adapter','use_v11_transfer_adapter','enable_adaptive_range_and_retry'}:changed=_enable_adaptive_transfer(manifest,parallel=True)
...
def main()->int:
    ...
    active,error=reconcile_failure(queue,status);entry,signature=match_entry(error,dictionary);maximum=int(manifest.get('recoveryPolicy',{}).get('maxAttemptsPerItem',manifest.get('maxAttemptsPerItem',3)));attempts=int((active or {}).get('attemptCount',0));action='none';retry=False;requires_human=False;cooldown=0;changed={}
    if active:
        if attempts>=maximum:action='block_after_attempt_limit';requires_human=True
            ...
            if retry:changed=apply_action(action,active,manifest);active['state']='pending';active.pop('error',None);active['lastRecoveryAt']=now();active['lastRecoveryAction']=action;queue['status']='recovery_scheduled';queue.pop('activeExternalSourceId',None)
            else:queue['status']='blocked'
    report={'schemaVersion':4,'generatedAt':now(),'dictionaryVersion':dictionary.get('version'),'dictionaryEntryId':(entry or {}).get('id'),'manifest':manifest_path.relative_to(ROOT).as_posix(),'activeExternalSourceId':(active or {}).get('externalSourceId'),'category':(entry or {}).get('layer','unknown'),'matchedSignature':signature,'diagnosis':(entry or {}).get('diagnosis','No safe deterministic match was found.'),'action':action,'retryScheduled':retry,'retryDelaySeconds':cooldown,'requiresHumanReview':requires_human,'attemptCount':attempts,'maxAttempts':maximum,'changedRuntimeSettings':changed,'errorExcerpt':error[-3000:],'safety':{'sourceCodeModified':False,'authorizationBroadened':False,'mediaRetentionChanged':False,'queueScopeExpanded':False}}
