#!/usr/bin/env python3
"""Run exactly 500 distinct heartbeat supervision scenarios for a ten-item serial queue."""
...
def valid_serial(queue: dict) -> bool:
    ...
    return (
        ...
        and [item["sequence"] for item in items] == sorted(item["sequence"] for item in items)
...
for i in range(100):
    age = i % 90
    report, queue, projection = supervisor.evaluate(config, manifest, base_queue(), {"updatedAt": stamp(1)}, runtime("p012", state="running", stage="analysis", age=age, sequence=i + 1), dictionary, {}, NOW)
...
for i in range(100):
    age = 91 + (i % 89)
    report, queue, projection = supervisor.evaluate(config, manifest, base_queue(), {"updatedAt": stamp(1)}, runtime("p012", state="running", stage="analysis", age=age, sequence=i + 101), dictionary, {}, NOW)
...
for i in range(100):
    age = 180 + i
    report, queue, projection = supervisor.evaluate(config, manifest, base_queue(), {"updatedAt": stamp(1)}, runtime("p012", state="running", stage="persisting", age=age, sequence=i + 201), dictionary, {}, NOW)
...
for i in range(100):
    age = i
    report, queue, projection = supervisor.evaluate(config, manifest, base_queue(first_state="imported"), {"updatedAt": stamp(1)}, runtime("p012", state="running", stage="persisting", age=age, sequence=i + 301), dictionary, {}, NOW)
    record(f"terminal-{i:03d}", report["decision"] == "stale_terminal_heartbeat" and report["resumeWorkflow"] and not report["queueChanged"] and projection is not None and projection["externalSourceId"] == "p013" and projection["state"] == "queued" and valid_serial(queue), f"age={age}")

# 100 duplicate resume requests inside cooldown must be suppressed.
for i in range(100):
    previous = {"resumeTargetExternalSourceId": "p013", "lastResumeRequestedAt": stamp(i)}
    report, queue, projection = supervisor.evaluate(config, manifest, base_queue(first_state="imported"), {"updatedAt": stamp(1)}, runtime("p013", state="queued", stage="queued", age=400 + i, sequence=i + 401), dictionary, previous, NOW)
    record(f"dedup-{i:03d}", report["decision"] == "pending_queued" and not report["resumeWorkflow"] and report["resumeSuppressedByCooldown"] and report["resumeTargetExternalSourceId"] == "p013" and projection is None and not report["queueChanged"] and valid_serial(queue), f"elapsed={i}")
...
print("Heartbeat ten-item serial supervision matrix: 500/500 checks passed")
